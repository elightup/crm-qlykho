...
<div class="wrap">
	<div class="crm-list detail-order">
		<div id="" class="crm-table">
					...
					<tr class="text-gray-700 dark:text-gray-400" data-order="<?= esc_attr( $order->id ) ?>">
						...
						<td class="px-4 py-3">
							<select name="trang-thai" id="">
								<option value="<?= esc_attr( $order->trang_thai ) ?>"><?= esc_html( $order->trang_thai ) ?></option>
			...
			<table class="table table-striped w-full overflow-hidden shadow-xs mb-8">
				...
				<tbody class="list data-list bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
					...
					foreach ( $list_product as $product_id => $product ) :
						...
						?>
						<tr class="text-gray-700 dark:text-gray-400" data-order="<?= esc_attr( $product_id ) ?>">
			...
			<button class="px-4 py-2 text-sm font-medium text-white transition-colors bg-blue-600 border border-transparent rounded-lg active:bg-blue-600 focus:outline-none focus:shadow-outline-purple save_product">Cập nhật</button>
		</div>
		...
		<legend><h2 class="title-action mt-4 mb-4 text-lg font-semibold text-gray-600 dark:text-gray-300">Thêm sản phẩm vào đơn</h2></legend>
					...
					<div class="add-product">
						<select name="product_name" id="product__name" class="rwmb">
							...
							foreach ( $sanpham as $sp ) :
								?>
								<option value="<?= esc_attr( $sp->id );?>"  ><?= esc_html( $sp->ten );?></option>
								<?php
							endforeach;
							?>
						</select>
						<div class="product-number">
							<input type="number" name="number_product" id="number_product" class="rwmb" style="width: 5rem;">
							<button class="popup-kho px-4 py-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-blue-600 border border-transparent rounded-lg active:bg-blue-600 focus:outline-none focus:shadow-outline-purple btn_add_product" data-popup="product-4">
								Chọn kho
							</button>
						</div>
					</div>
			...
			<div class="action_btn">
				<button data-kho="<?= esc_attr( $id_kho );?>" class="px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-lg focus:outline-none focus:shadow-outline-purple add_product_kho">Thêm</button>
				<button data-kho="<?= esc_attr( $id_kho );?>" class="px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-lg focus:outline-none focus:shadow-outline-purple btn-clear ">Nhập lại</button>
			</div>
		</fieldset>
	</div>
</div>
